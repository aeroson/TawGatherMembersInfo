...
create procedure GetChildUnits(in rootUnitTawId bigint(20))
begin
	...
	insert into GetChildUnits_result 
		...
		(
			select u1.UnitId from Units u1 where u1.TawId = rootUnitTawId
		) a
		union all
		(
			select u2.UnitId from Units u1
				join Units u2 on u2.ParentUnit_UnitId = u1.UnitId and u1.TawId = rootUnitTawId 
		)
		union all
		(
